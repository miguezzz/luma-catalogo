    ...
    <>
      ...
      >
        <header className="p-4 flex justify-between items-center border-b">
          <h2 className="text-lg font-semibold md:text-2xl 2xl:text-3xl">Seu Carrinho</h2>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {items.length === 0 ? (
            <p className="text-gray-500 md:text-xl 2xl:text-2xl">Seu carrinho está vazio.</p>
          ) : (
            items.map((item) => (
        ...
        <footer className="p-4 border-t">
          <div className="flex justify-between items-center mb-4">
            <span className="text-lg font-semibold md:text-xl 2xl:text-2xl">Total:</span>
            <span className="text-lg font-semibold md:text-xl 2xl:text-2xl">
              R$ {items.reduce((total, item) => total + item.price * item.qty, 0).toFixed(2)}
            </span>
          </div>
          <button
            className="w-full bg-amber-600 text-white hover:bg-pink-400 hover:text-white border-none font-bold py-2 px-8 rounded-3xl transform hover:shadow-[0_0_30px_#ff5bef] transition-all ease-in-out duration-300 md:text-xl 2xl:text-xl"
